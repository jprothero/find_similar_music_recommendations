...
def npmi_batch_popularity_weighted(mat, i, alpha, beta, cache=None, eps=1e-14, derank_i=True, normalize=False):
    alpha = max(alpha, 0)
    beta = max(beta, 0)
...
def a_to_b_error_metric_npmi_pop_weighted(mat, a, b, alpha, beta, regularize, cache_a=None, cache_b=None, verbose=True):
    similarity_scores_a = npmi_batch_popularity_weighted(mat, a, alpha, beta, cache=cache_a)
    ranking_a = np.argsort(-similarity_scores_a)
    
    similarity_scores_b = npmi_batch_popularity_weighted(mat, b, alpha, beta, cache=cache_b)
    ranking_b = np.argsort(-similarity_scores_b)
    
    metric = (ranking_a.tolist().index(b) + ranking_b.tolist().index(a))/2
    loss = metric
    if regularize:
...
def optimize_alpha_using_a_to_b_matching_npmi(mat, a, b, regularize=False, verbose=True):
    def f(x, 
        ...
        return a_to_b_error_metric_npmi_pop_weighted(mat, a, b, alpha, beta, regularize,
